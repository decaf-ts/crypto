...
describe("CryptoService", () => {
  ...
  describe("extractKeyFromDerivedKey", () => {
    it("should extract salt and key from derived key", () => {
      const derivedKey =
        "AAAAAAAAAAAAAAAAAAAAAA" + // 16 bytes of zeros (salt)
  ...
  describe("encrypt / decrypt (simple API)", () => {
    ...
    it("should fail decryption with wrong secret", async () => {
      ...
      await expect(
        cryptoService.decrypt(result.encryptedData, wrongSecret, result.salt)
      ).rejects.toThrow();
    });
    ...
    it("should fail decryption with tampered data", async () => {
      ...
      await expect(
        cryptoService.decrypt(tampered, secret, result.salt)
      ).rejects.toThrow();
    });
  ...
  describe("encryptPayload / decryptPayload (low-level API)", () => {
    it("should encrypt and decrypt with pre-derived key", async () => {
      ...
      const decrypted = await cryptoService.decryptPayload(
        encryptedData,
        testKey
      );
      expect(decrypted).toBe(payload);
  ...
  describe("Integration: derive, encrypt, decrypt flow", () => {
    it("should derive key, encrypt with it, then decrypt with extracted key", async () => {
      ...
      const { encryptedData } = await cryptoService.encryptPayload(
        payload,
        "derived-key",
        key
      );
    ...
    it.skip("should demonstrate .for() API for switching between configs", async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        if (i % 2 === 0) {
          await baseService
            .for({ aesGcm: { length: 128 } })
            .encrypt(payload, secret);
        } else {
          await baseService
            .for({ aesGcm: { length: 192 } })
            .encrypt(payload, secret);
        }
      }
      ...
      console.log(
        `Using .for() to switch configs: ${mixedConfigTime}ms vs ${singleConfigTime}ms for separate services`
      );
    });
  });
  ...
  describe("Error handling", () => {
    it("should throw InternalError on encryption failure with invalid key", async () => {
      ...
      await expect(
        cryptoService.encryptPayload(payload, "key", invalidKey)
    ...
    it("should throw InternalError on decryption failure with invalid data", async () => {
      ...
      await expect(
        cryptoService.decryptPayload(invalidData, testKey)
  ...
  describe("Configuration", () => {
    it("should use custom AES-GCM length and IV length when configured", async () => {
      ...
      const { encryptedData, metadata } =
        await cryptoServiceCustom.encryptPayload(
          payload,
          "custom-key",
          testKey
        );
      ...
      // Decrypt and verify
      const decrypted = await cryptoServiceCustom.decryptPayload(
        encryptedData,
        testKey
      );
      expect(decrypted).toBe(payload);
